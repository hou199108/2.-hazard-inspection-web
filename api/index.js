...
async function getDatabase() {
  ...
  dbPromise = (async () => {
    ...
    // 初始化隐患类型数据（分块加载，防止Vercel超时）
    try {
      const chunkDir = path.join(__dirname, '..');
      let totalLoaded = 0;
      for (let i = 0; i < 20; i++) {
        const chunkPath = path.join(chunkDir, `hazard_chunk_${i}.sql`);
        if (!fs.existsSync(chunkPath)) break;
        const sqlContent = fs.readFileSync(chunkPath, 'utf8').trim();
        if (sqlContent) {
          ...
          totalLoaded = db.prepare('SELECT COUNT(*) as count FROM hazard_types').get().count;
          console.log(`  📦 第${i+1}块: ${sqlContent.match(/\(/g).length}条 (累计${totalLoaded})`);
        }
      }
      if (totalLoaded > 0) {
        console.log(`✅ 已加载 ${totalLoaded} 条隐患类型数据`);
      } else {
        // Fallback: try single sql file
        const fallbackPaths = [
          path.join(__dirname, '..', 'init_hazard_types.sql'),
          path.join(__dirname, '..', '..', 'init_hazard_types.sql'),
        ...
        for (const fp of fallbackPaths) {
          if (fs.existsSync(fp)) {
            ...
            if (sqlContent) {
              ...
              console.log(`✅ 已加载 ${count} 条隐患类型数据（单文件）`);
            }
            break;
          }
        }
      ...
      console.error('⚠️ 隐患类型数据加载失败:', err.message);
    }
